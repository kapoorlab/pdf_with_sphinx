...
from docutils.writers.latex2e import LaTeXTranslator, PreambleCmds, Writer
...
class Translator(LaTeXTranslator):
    ...
    def footmark(self, n):
        ...
        """
        return (
            "\\setcounter{footnotecounter}{%d}" % n,
    ...
    def depart_document(self, node):
        ...
        for n, auth in enumerate(self.author_names):
            ...
            authors += [
                ...
                % {"author": auth, "footmark": footmarks}
            ]

            if (
                auth in self.equal_contributors
                and equal_authors_mentioned == False
            ):
            ...
            for inst in self.author_institution_map[auth]:
                if not inst in institutions_mentioned:
                    ...
                    authors[-1] += (
                        r"%(footmark_counter)s\thanks{%(footmark)s %(institution)s}"
                        % {
                            "footmark_counter": fm_counter,
                            "footmark": fm,
                            "institution": inst,
    ...
    def visit_figure(self, node):
        ...
        self.out.append("\\begin{{{}}}{}".format(self.figure_type, placements))
    ...
    def visit_image(self, node):
        ...
        if scale is not None:
            figure_opts.append("scale=%.2f" % (scale / 100.0))
        ...
        self.out.append(r"\noindent\makebox[{}][{}]".format(width, align[0]))
        self.out.append(
            r"{{\includegraphics[{}]{{{}}}}}".format(
                ",".join(figure_opts), filename
            )
        )
    ...
    def validate_orcid(self, orcid):
        ...
        if len(groups) != 4:
            raise ValueError(
                "ORCID {} must have four groups of digits".format(orcid)
            )
        ...
        if not ((checksum_char == "X") or checksum_char.isdigit()):
            raise ValueError(
                'ORCID {} last character must be digit or "X"'.format(orcid)
            )
        total = 0
        for c in first15:
            total = (total + int(c)) * 2
        ...
        if (checksum_char == "X" and result != 10) or (
            ...
            raise ValueError(
                "ORCID {} has last char {} and checksum {}".format(
                    orcid, checksum_char, result
                )
            )
        return
