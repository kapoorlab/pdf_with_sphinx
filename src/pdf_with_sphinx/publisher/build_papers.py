...
def paper_stats(paper_id, start, doi_prefix=None):
    ...
    stats = options.cfg2dict(
        os.path.join(output_dir, paper_id, "paper_stats.json")
    )
    ...
    print('"{}" from p. {} to {}'.format(paper_id, start, stop))

    # Build table of contents
    stats.update(
        {
            "page": {"start": start, "stop": stop},
            "paper_id": paper_id,
...
def other_stats(track_dir, _id, doi_prefix=None):
    ...
    # Build table of contents
    stats.update({"slide_id": _id, "doi": doi if is_final else ""})
...
if __name__ == "__main__":
    ...
    scipy_entry["proceedings"]["doi"] = (
        make_doi(doi_prefix) if is_final else ""
    )
