...
import options
from conf import output_dir, papers_dir, status_file
...
def rst2tex(in_path, out_path):
    ...
    try:
        ...
        warnings.warn(
            RuntimeWarning(
                "Could not import Pygments. " "Syntax highlighting will fail."
            )
        )
    ...
    settings = {
        ...
        "documentoptions": "letterpaper,compsoc,twoside,onecolumn",
    ...
    try:
        ...
        raise RuntimeError(
            "Found more than one input .rst--not sure which " "one to use."
        )

    with open(rst, encoding="utf-8") as f:
        content = header + f.read()

    tex = dc.publish_string(
        source=content, writer=writer, settings_overrides=settings
    )
    ...
    with open(tex_file, mode="wb") as f:
        try:
...
def page_count(pdflatex_stdout, paper_dir):
    ...
    regexp = re.compile(rb"Output written on paper.pdf \((\d+) pages")
...
def build_paper(paper_id, start=1):
    ...
    with open(page_number_file, "w", encoding="utf-8") as f:
        f.write(r"\setcounter{page}{%s}" % start)
