...
class XrefMeta:
    ...
    def make_batch(self):
        ...
        batch = xml.Element(
            ...
            attrib={
                location: "http://www.crossref.org/schema/4.4.2 http://www.crossref.org/schemas/crossref4.4.2.xsd"
            },
        )
        ...
        depositor_name.text = self.scipy_entry["proceedings"]["xref"][
            "depositor_name"
        ]
        depositor_email_address = xml.SubElement(depositor, "email_address")
        depositor_email_address.text = self.scipy_entry["proceedings"]["xref"][
            "depositor_email"
        ]
    ...
    def make_conference(self, body):
        ...
        conference_name.text = self.scipy_entry["proceedings"]["title"][
            "conference"
        ]
        conference_acronym = xml.SubElement(
            event_metadata, "conference_acronym"
        )
        conference_acronym.text = self.scipy_entry["proceedings"]["title"][
            "acronym"
        ]
        conference_number = xml.SubElement(event_metadata, "conference_number")
        conference_number.text = self.scipy_entry["proceedings"]["title"][
            "ordinal"
        ]
        conference_location = xml.SubElement(
            event_metadata, "conference_location"
        )
        ...
        conference_date.text = " ".join(
            [
                self.scipy_entry["proceedings"]["dates"],
                self.scipy_entry["proceedings"]["year"],
            ]
        )
    ...
    def make_conference_proceedings(self, conference):
        ...
        """
        proceedings_series_metadata = xml.SubElement(
            conference, "proceedings_series_metadata"
        )
        series_metadata = xml.SubElement(
            proceedings_series_metadata, "series_metadata"
        )
        ...
        original_language_title = xml.SubElement(
            titles, "original_language_title"
        )
        original_language_title.text = self.scipy_entry["series"]["title"][
            "full"
        ]
        ...
        series_resource.text = self.scipy_entry["series"]["xref"][
            "resource_url"
        ]
        proceedings_title = xml.SubElement(
            proceedings_series_metadata, "proceedings_title"
        )
        proceedings_title.text = self.scipy_entry["proceedings"]["title"][
            "full"
        ]
        proceedings_subject = xml.SubElement(
            proceedings_series_metadata, "proceedings_subject"
        )
        proceedings_subject.text = (
            "Scientific Computing with Python"  # TODO: move to scipy_proc.json
        )
        ...
        publication_date = xml.SubElement(
            proceedings_series_metadata, "publication_date"
        )
        publication_year = xml.SubElement(publication_date, "year")
        publication_year.text = self.scipy_entry["proceedings"]["year"]
        noisbn = xml.SubElement(
            proceedings_series_metadata, "noisbn", reason="simple_series"
        )  # Do not modify, unless someone has actually gone and gotten us an ISBN
        proceedings_doi_data = xml.SubElement(
            proceedings_series_metadata, "doi_data"
        )
    ...
    def make_conference_papers(self, conference, entry):
        ...
        for index, contributor in enumerate(entry.get("author", [])):
            ...
            person_name = xml.SubElement(
                paper_contributors,
                "person_name",
                contributor_role="author",
                sequence="additional" if index else "first",
        ...
        publication_date = xml.SubElement(
            paper, "publication_date", media_type="print"
        )
    ...
    def make_database(self, body):
        ...
        database_metadata = xml.SubElement(
            database, "database_metadata", language="en"
        )
        ...
        first_name, last_name = split_name(
            self.scipy_entry["proceedings"]["xref"]["depositor_name"]
        )
        given_name = xml.SubElement(person_name, "given_name")
        given_name.text = first_name
        surname = xml.SubElement(person_name, "surname")
    ...
    def make_dataset(self, database, entry):
        ...
        description.text = entry.get(
            "description", "Material presented at SciPy"
        )
        program = xml.SubElement(
            dataset,
            "program",
            name="relations",
        ...
        inter_work_relation = xml.SubElement(
            related_item,
            "inter_work_relation",
            attrib={"relationship-type": "isPartOf", "identifier-type": "doi"},
        )
